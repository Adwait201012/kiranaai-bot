...
async function sendMorningMessageToOwner({ owner_phone, shop_name }) {
  try {
    ...
    const shopLabelForEmp = shopLabel;
    await Promise.allSettled(
      (employees || []).map((emp) => {
        const empMsg =
          `🌅 Suprabhat!\n\n` +
          `Aaj bhi ${shopLabelForEmp} ke liye mehnat karte rahein. 💪\n` +
          `Koi bhi udhaar add karne ke liye seedha likh dein!`;
        return sendTextMessage({ to: emp.employee_phone, text: empMsg });
      })
    );
  } catch (err) {
...
async function sendEveningMessageToOwner({ owner_phone, shop_name }) {
  try {
...
async function processOwnersInBatches(jobFn) {
  let from = 0;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    if (error) {
      console.error("processOwnersInBatches fetch failed:", error.message);
      break;
    }
    ...
    await Promise.allSettled(data.map((owner) => jobFn(owner)));
...
async function sendMorningMessages() {
  ...
  await processOwnersInBatches(sendMorningMessageToOwner);
}

async function sendEveningMessages() {
  console.log("[Scheduled] Running evening messages...");
  await processOwnersInBatches(sendEveningMessageToOwner);
}
